...
class StealthManager:
    ...
    WDA_EXCLUDEFROMCAPTURE = 0x00000011  # Windows 10 2004+ 最强防护
    
    GWL_EXSTYLE = -20
    WS_EX_TOOLWINDOW = 0x00000080  # 从Alt-Tab隐藏
    WS_EX_LAYERED = 0x00080000     # 分层窗口(用于透明度)
    ...
    def set_capture_protection(self, hwnd: int, enable: bool = True) -> bool:
        ...
        使用 WDA_EXCLUDEFROMCAPTURE 使窗口从所有屏幕捕获中完全消失
        (不是变黑，是透明/不存在)
        
        Args:
            hwnd: 窗口句柄
            enable: True启用保护，False禁用
